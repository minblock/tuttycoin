...
const char * const BITCOIN_CONF_FILENAME = "tuttycoin.conf";
...
#else
    const char* pszModule = "tuttycoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Tuttycoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Tuttycoin";
#else
    // Unix
    return pathRet / ".tuttycoin";
#endif
#endif
}
